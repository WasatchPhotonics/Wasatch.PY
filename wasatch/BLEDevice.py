...
class BLEDevice(InterfaceDevice):
    ...
    def _init_process_funcs(self):
        ...
        process_f['integration_time_ms'
            ] = lambda x: asyncio.run_coroutine_threadsafe(self.
            _set_integration_time_ms(x), self.loop)
        process_f['detector_gain'
            ] = lambda x: asyncio.run_coroutine_threadsafe(self._set_gain(x
            ), self.loop)
        process_f['laser_enable'] = lambda x: asyncio.run_coroutine_threadsafe(
            self._set_laser(x), self.loop)
        process_f['vertical_binning'
            ] = lambda x: asyncio.run_coroutine_threadsafe(self.
            _set_vertical_roi(x), self.loop)
        return process_f
    ...
    async def _set_laser(self, value: int) ->SpectrometerResponse:
        ...
        try:
            ...
            return SpectrometerResponse(False, error_msg=
                'error setting laser', error_lvl=ErrorLevel.high)
        return SpectrometerResponse(True)

    async def _set_vertical_roi(self, lines):
        ...
        try:
            if not self.settings.is_micro():
                ...
                return SpectrometerResponse(False, error_msg=
                    'ROI not supported', error_lvl=ErrorLevel.low)
            try:
                start = lines[0]
                end = lines[1]
            except:
                ...
                return SpectrometerResponse(False, error_msg=
                    'invalid start stop lines', error_lvl=ErrorLevel.low)
            if start < 0 or end < 0:
                log.error(
            ...
            log.error(
                f'error trying to set vertical over ble of {e} with values {start} and {end}'
                )
            return SpectrometerResponse(False, error_msg=
                'error trying to set roi', error_lvl=ErrorLevel.low)
        return SpectrometerResponse(True)

    async def _set_integration_time_ms(self, value: int
        ...
        try:
            ...
            return SpectrometerResponse(False, error_msg=
                'error setting int time', error_lvl=ErrorLevel.low)
        return SpectrometerResponse(True)
    ...
    async def _set_gain(self, value: int) ->SpectrometerResponse:
        ...
        try:
            msb = int(value)
            lsb = int((value - int(value)) * 256) & 255
            ...
            return SpectrometerResponse(False, error_msg=
                'error trying to write gain', error_lvl=ErrorLevel.medium)
        return SpectrometerResponse(True)
    ...
    async def _ble_acquire(self) ->SpectrometerResponse:
        ...
        request = await self.client.write_gatt_char(DEVICE_ACQUIRE_UUID,
            bytes(0), response=True)
        ...
        for loop_index in range(0, loop_count):
            ...
            if self.disconnect:
                log.debug('ble spec is set to disconnect, returning False')
                log.info(
                    'Disconnecting, stopping spectra acquire and returning None'
                    )
                return SpectrometerResponse(False)
            if request_retry:
                self.retry_count += 1
            ...
            if self.retry_count == 1:
                delay_ms = int(self.settings.state.integration_time_ms *
                    THROWAWAY_SPECTRA)
            ...
            request_retry = False
            log.debug(
                f'requesting spectrum packet starting at pixel {self.pixels_read}'
                )
            ...
            log.debug(
                f'received spectrum packet starting at pixel {first_pixel} with {pixels_in_packet} pixels'
                )
            for i in range(pixels_in_packet):
                ...
                if self.pixels_read == pixels:
                    ...
                    if i + 1 != pixels_in_packet:
                        log.error(
                            f'ignoring {pixels_in_packet - (i + 1)} trailing pixels'
                            )
                    break
            response = None
            for i in range(4):
    ...
    async def _get_eeprom(self):
        ...
        for i in range(EEPROM.MAX_PAGES):
            ...
            for j in range(EEPROM.SUBPAGE_COUNT):
                ...
                log.debug(
                    f'Writing to tell gateway to get page {i} ands subpage {j}'
                    )
                request = await self.client.write_gatt_char(
                    SELECT_EEPROM_PAGE_UUID, page_ids, response=True)
    ...
    def connect(self):
        self.thread = threading.Thread(target=self.loop.run_forever, daemon
            =True)
        self.thread.start()
        ...
        self.label = (
            f'{self.settings.eeprom.serial_number} ({self.settings.eeprom.model})'
            )
        return SpectrometerResponse(True)
    ...
    def close(self):
        ...
        fut = asyncio.run_coroutine_threadsafe(self._disconnect_spec(),
            self.loop)
        result = fut.result()
        self.loop.stop()
