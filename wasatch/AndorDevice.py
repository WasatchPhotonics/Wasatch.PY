...
class AndorDevice(InterfaceDevice):
    ...
    def __init__(self, device_id, message_queue=None):
        ...
        dll_paths = ['C:\\Program Files\\Andor Driver Pack 2',
            'C:\\Program Files\\Andor SDK', 'dist\\Andor', 'dist']
        ...
        self.config_names_to_eeprom = {'wp_serial_number': 'serial_number',
            'wp_model': 'model'}
    ...
    def _init_process_funcs(self):
        ...
        process_f['detector_tec_setpoint_degC'
            ] = lambda x: self.set_tec_setpoint(int(round(x)))
        return process_f
    ...
    def _get_spectrum_raw(self):
        ...
        if success != self.SUCCESS:
            log.debug(
                f'getting spectra did not succeed. Received code of {success}. Returning'
                )
            return
        ...
        for loop_index in range(0, loop_count):
            ...
            if self.settings.eeprom.has_cooling and self.toggle_state:
                ...
                if self.SUCCESS != result:
                    ...
                    log.debug(
                        f'andor read temperature, value of {c_temp.value}')
            ...
            try:
                ...
                log.error(
                    f'Andor Device: encountered USB error in reading for device {self.device}'
                    )
            if reading.spectrum is None or reading.spectrum == []:
                if self.failure_count > 3:
                    return SpectrometerResponse(data=False, error_msg=
                        'exceeded failure for readings')
    ...
    def _close_ex_shutter(self):
        self.check_result(self.driver.SetShutterEx(1, 1, self.
            SHUTTER_SPEED_MS, self.SHUTTER_SPEED_MS, 2), 'SetShutterEx(2)')
        self.settings.state.shutter_enabled = False
        return SpectrometerResponse(True)

    def _open_ex_shutter(self):
        self.check_result(self.driver.SetShutterEx(1, 1, self.
            SHUTTER_SPEED_MS, self.SHUTTER_SPEED_MS, 1), 'SetShutterEx(1)')
    ...
    def connect(self):
        if self.dll_fail:
            return SpectrometerResponse(False, error_msg=
                "can't find Andor DLL; please confirm Andor Driver Pack 2 installed"
                )
        cameraHandle = c_int()
        self.check_result(self.driver.GetCameraHandle(self.spec_index,
            byref(cameraHandle)), 'GetCameraHandle')
        self.check_result(self.driver.SetCurrentCamera(cameraHandle.value),
            'SetCurrentCamera')
        try:
            path_to_ini = create_string_buffer(b'\x00' * 256)
            self.check_result(self.driver.Initialize(path_to_ini), 'Initialize'
                )
        except:
            log.error('Andor.Initialize failed', exc_info=1)
            return SpectrometerResponse(False, error_msg=
                'Andor initialization failed')
        self.get_serial_number()
        self.init_tec_setpoint()
        self.init_detector_area()
        if not self._check_config_file():
            self.config_values = {'detector_serial_number': self.serial,
                'wavelength_coeffs': [0, 1, 0, 0], 'excitation_nm_float': 0,
                'raman_intensity_coeffs': [],
                'raman_intensity_calibration_order': 0, 'invert_x_axis': False}
            log.debug(
                f'connect: config file not found, so defaulting to these: {self.config_values}'
                )
            self.save_config()
        else:
        ...
        self.check_result(self.driver.SetAcquisitionMode(1),
            'SetAcquisitionMode(single_scan)')
        self.check_result(self.driver.SetTriggerMode(0), 'SetTriggerMode')
        self.check_result(self.driver.SetReadMode(0),
            'SetReadMode(full_vertical_binning)')
        self.init_detector_speed()
        self.check_result(self.driver.SetShutterEx(1, 1, self.
            SHUTTER_SPEED_MS, self.SHUTTER_SPEED_MS, 0),
            'SetShutterEx(fully automatic external with internal always open)')
        self.settings.state.shutter_enabled = True
    ...
    def _load_config_values(self):
        ...
        for k in ['model', 'detector', 'serial_number', 'invert_x_axis',
            'wavelength_coeffs', 'excitation_nm_float',
            'raman_intensity_coeffs', 'raman_intensity_calibration_order',
            'startup_temp_degC', 'startup_integration_time_ms']:
            if k in self.config_values:
    ...
    def set_integration_time_ms(self, ms):
        ...
        kinetic = c_float()
        sec = ms / 1000.0
        self.check_result(self.driver.SetExposureTime(c_float(sec)),
            f'SetExposureTime({sec})')
        self.check_result(self.driver.GetAcquisitionTimings(byref(exposure),
            byref(accumulate), byref(kinetic)), 'GetAcquisitionTimings')
        log.debug(
            f'read integration time of {exposure.value:.3f}sec (expected {ms}ms)'
            )
        return SpectrometerResponse(data=True)

    def get_serial_number(self):
        sn = c_int()
        self.check_result(self.driver.GetCameraSerialNumber(byref(sn)),
    ...
    def init_tec_setpoint(self):
        minTemp = c_int()
        maxTemp = c_int()
        self.check_result(self.driver.GetTemperatureRange(byref(minTemp),
            byref(maxTemp)), 'GetTemperatureRange')
        ...
        self.check_result(self.driver.SetTemperature(self.setpoint_deg_c),
            f'SetTemperature({self.setpoint_deg_c})')
        log.debug(
            f'set TEC to {self.setpoint_deg_c}°C (range {self.settings.eeprom.min_temp_degC}, {self.settings.eeprom.max_temp_degC})'
            )
        return SpectrometerResponse(True)
    ...
    def set_tec_setpoint(self, set_temp):
        if (set_temp < self.settings.eeprom.min_temp_degC or set_temp >
            self.settings.eeprom.max_temp_degC):
            log.error(
                f'requested temp of {set_temp}, but it is outside range ({self.settings.eeprom.min_temp_degC}C, {self.settings.eeprom.max_temp_degC}C)'
                )
            return
    ...
    def init_detector_area(self):
        xPixels = c_int()
        yPixels = c_int()
        self.check_result(self.driver.GetDetector(byref(xPixels), byref(
            yPixels)), 'GetDetector(x, y)')
    ...
    def _obtain_gain_info(self):
        ...
        log.debug(
            f'obtained gain options for spec, values were {self.gain_options}')

    def init_detector_speed(self):
        speed = c_float()
        if self.height > 1:
            VSnumber = c_int()
            self.check_result(self.driver.GetFastestRecommendedVSSpeed(
                byref(VSnumber), byref(speed)), 'GetFastestRecommendedVSSpeed')
            self.check_result(self.driver.SetVSSpeed(VSnumber.value),
                f'SetVSSpeed({VSnumber.value})')
        else:
        ...
        HSnumber = 0
        ADnumber = 0
        self.check_result(self.driver.GetNumberADChannels(byref(nAD)),
            'GetNumberADChannels')
        for iAD in range(nAD.value):
            self.check_result(self.driver.GetNumberHSSpeeds(iAD, 0, byref(
                sIndex)), f'GetNumberHSSpeeds({iAD})')
            for iSpeed in range(sIndex.value):
                self.check_result(self.driver.GetHSSpeed(iAD, 0, iSpeed,
                    byref(speed)), f'GetHSSpeed(iAD {iAD}, iSpeed {iSpeed})')
                if speed.value > STemp:
                    STemp = speed.value
                    HSnumber = iSpeed
                    ADnumber = iAD
        self.check_result(self.driver.SetADChannel(ADnumber),
            f'SetADChannel({ADnumber})')
        self.check_result(self.driver.SetHSSpeed(0, HSnumber),
            f'SetHSSpeed({HSnumber})')
        log.debug(
            f'set AD channel {ADnumber} with horizontal speed {HSnumber} ({STemp})'
            )
        return SpectrometerResponse(True)
    ...
    def load_error_codes(self):
        self.error_codes = {(20001): 'DRV_ERROR_CODES', (20002):
            'DRV_SUCCESS', (20003): 'DRV_VXDNOTINSTALLED', (20004):
            'DRV_ERROR_SCAN', (20005): 'DRV_ERROR_CHECK_SUM', (20006):
            'DRV_ERROR_FILELOAD', (20007): 'DRV_UNKNOWN_FUNCTION', (20008):
            'DRV_ERROR_VXD_INIT', (20009): 'DRV_ERROR_ADDRESS', (20010):
            'DRV_ERROR_PAGELOCK', (20011): 'DRV_ERROR_PAGEUNLOCK', (20012):
            'DRV_ERROR_BOARDTEST', (20013): 'DRV_ERROR_ACK', (20014):
            'DRV_ERROR_UP_FIFO', (20015): 'DRV_ERROR_PATTERN', (20017):
            'DRV_ACQUISITION_ERRORS', (20018): 'DRV_ACQ_BUFFER', (20019):
            'DRV_ACQ_DOWNFIFO_FULL', (20020):
            'DRV_PROC_UNKONWN_INSTRUCTION', (20021): 'DRV_ILLEGAL_OP_CODE',
            (20022): 'DRV_KINETIC_TIME_NOT_MET', (20023):
            'DRV_ACCUM_TIME_NOT_MET', (20024): 'DRV_NO_NEW_DATA', (20025):
            'DRV_PCI_DMA_FAIL', (20026): 'DRV_SPOOLERROR', (20027):
            'DRV_SPOOLSETUPERROR', (20028): 'DRV_FILESIZELIMITERROR', (
            20029): 'DRV_ERROR_FILESAVE', (20033): 'DRV_TEMPERATURE_CODES',
            ...
            'DRV_TEMP_STABILIZED', (20037): 'DRV_TEMP_NOT_REACHED', (20038):
            'DRV_TEMP_OUT_RANGE', (20039): 'DRV_TEMP_NOT_SUPPORTED', (20040
            ): 'DRV_TEMP_DRIFT', (20049): 'DRV_GENERAL_ERRORS', (20050):
            'DRV_INVALID_AUX', (20051): 'DRV_COF_NOTLOADED', (20052):
            'DRV_FPGAPROG', (20053): 'DRV_FLEXERROR', (20054):
            'DRV_GPIBERROR', (20055): 'DRV_EEPROMVERSIONERROR', (20064):
            'DRV_DATATYPE', (20065): 'DRV_DRIVER_ERRORS', (20066):
            'DRV_P1INVALID', (20067): 'DRV_P2INVALID', (20068):
            'DRV_P3INVALID', (20069): 'DRV_P4INVALID', (20070):
            'DRV_INIERROR', (20071): 'DRV_COFERROR', (20072):
            'DRV_ACQUIRING', (20073): 'DRV_IDLE', (20074): 'DRV_TEMPCYCLE',
            (20075): 'DRV_NOT_INITIALIZED', (20076): 'DRV_P5INVALID', (
            20077): 'DRV_P6INVALID', (20078): 'DRV_INVALID_MODE', (20079):
            ...
            'DRV_RANDOM_TRACK_ERROR', (20095): 'DRV_INVALID_TRIGGER_MODE',
            (20096): 'DRV_LOAD_FIRMWARE_ERROR', (20097):
            'DRV_DIVIDE_BY_ZERO_ERROR', (20098):
            'DRV_INVALID_RINGEXPOSURES', (20099): 'DRV_BINNING_ERROR', (
            20100): 'DRV_INVALID_AMPLIFIER', (20101):
            'DRV_INVALID_COUNTCONVERT_MODE', (20102):
            'DRV_USB_INTERRUPT_ENDPOINT_TIMEOUT', (20990):
            'DRV_ERROR_NOCAMERA', (20991): 'DRV_NOT_SUPPORTED', (20992):
            'DRV_NOT_AVAILABLE', (20115): 'DRV_ERROR_MAP', (20116):
            'DRV_ERROR_UNMAP', (20117): 'DRV_ERROR_MDL', (20118):
            ...
            20151): 'DRV_OWMEMORY_BAD_ADDR', (20152):
            'DRV_OWCMD_NOT_AVAILABLE', (20153): 'DRV_OW_NO_SLAVES', (20154):
            'DRV_OW_NOT_INITIALIZED', (20155): 'DRV_OW_ERROR_SLAVE_NUM', (
            20156): 'DRV_MSTIMINGS_ERROR', (20173): 'DRV_OA_NULL_ERROR', (
            ...
            'DRV_OA_INVALID_FILE', (20183): 'DRV_OA_FILE_HAS_BEEN_MODIFIED',
            (20184): 'DRV_OA_BUFFER_FULL', (20185):
            'DRV_OA_INVALID_STRING_LENGTH', (20186):
            ...
            'DRV_OA_FAILED_TO_GET_MODE', (20196):
            'DRV_OA_CAMERA_NOT_AVAILABLE', (20211): 'DRV_PROCESSING_FAILED'}
